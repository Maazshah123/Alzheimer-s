...
const normalizeMlBase = (raw: string | undefined): string | undefined => {
  ...
  for (const suffix of ["/health", "/predict/mri", "/predict", "/docs", "/openapi.json"]) {
    if (u.toLowerCase().endsWith(suffix.toLowerCase())) u = u.slice(0, -suffix.length).replace(/\/+$/, "");
  }
  return u || undefined;
};

const defaultBase = () => normalizeMlBase(import.meta.env.VITE_ML_API_URL as string | undefined);
const mriBase = () => normalizeMlBase(import.meta.env.VITE_ML_MRI_API_URL as string | undefined);
...
export type DoctorScanPanelProps = {
  preselectImageDataUrl?: string | null;
  onConsumedPreselect?: () => void;
  cardTitle: string;
  description: string;
  predictPath: string;
  /** If unset: CWT uses VITE_ML_API_URL; MRI uses VITE_ML_MRI_API_URL if set, else same as CWT. */
  mlBaseUrl?: string;
  runButtonLabel?: string;
  fileInputId: string;
};

const DoctorScanPanel = ({
  preselectImageDataUrl,
  onConsumedPreselect,
  cardTitle,
  description,
  predictPath,
  mlBaseUrl: mlBaseProp,
  runButtonLabel = "Run model",
  fileInputId,
}: DoctorScanPanelProps) => {
  ...
  const mlBase = useMemo(() => {
    if (mlBaseProp?.trim()) return normalizeMlBase(mlBaseProp);
    if (predictPath.includes("mri")) return mriBase() ?? defaultBase();
    return defaultBase();
  }, [mlBaseProp, predictPath]);
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        toast({
          title: "Image loaded",
          description: `Press ${runButtonLabel} to see the result and confidence.`,
        });
      } catch {
        if (!cancelled) {
  ...
  }, [preselectImageDataUrl, onConsumedPreselect, toast, runButtonLabel]);
  ...
  const runPredict = async () => {
    ...
    if (!file) {
      toast({
        ...
        description: "Upload an image file to analyze.",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      const res = await fetch(`${mlBase.replace(/\/$/, "")}${path}`, { method: "POST", body: fd });
    ...
    <Card className="rounded-xl border-0 bg-[#F0F2F4] shadow-sm">
      <CardHeader>
        <div className="flex items-center gap-2 border-b border-gray-200/90 pb-3">
          ...
          <CardTitle className="text-gray-900 font-bold border-0 pb-0">{cardTitle}</CardTitle>
        </div>
        <CardDescription className="text-gray-500 pt-3">{description}</CardDescription>
      </CardHeader>
      <CardContent className="space-y-6">
        <div className="space-y-2 max-w-xl">
          <Label htmlFor={fileInputId}>Image file</Label>
          <Input id={fileInputId} type="file" accept="image/*" onChange={(e) => setFile(e.target.files?.[0] ?? null)} />
        </div>
...
export default DoctorScanPanel;
